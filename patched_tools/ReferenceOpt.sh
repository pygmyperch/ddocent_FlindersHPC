...
if [[ "$ATYPE" == "RPE" || "$ATYPE" == "ROL" ]]; then
  ...
  cut -f1 -d "-" total.uniqs > total.u.F
  cut -f2 -d "-" total.uniqs > total.u.R
  ...
  rm -f total.uniqs total.u.* total.fr total.f.uniq*
else
...
# $sort -k1 -r -n "uniq.k.$CUTOFF.c.$CUTOFF2.seqs" | cut -f2 > totaluniqseq

# Skip combinations that produce no sequences
if [[ ! -s totaluniqseq ]]; then
  echo "WARNING: totaluniqseq is empty for K1=$CUTOFF K2=$CUTOFF2, skipping this combination" >&2
  rm -f totaluniqseq "uniq.k.$CUTOFF.c.$CUTOFF2.seqs"
  echo "NA"
  return 0
fi
...
LENGTH=$(mawk '!/^>/' uniq.full.fasta | mawk '(NR==1 || length($0) < shortest){shortest=length($0)} END {if (NR>0) print shortest}')

if [[ -z "${LENGTH:-}" || ! "${LENGTH}" =~ ^[0-9]+$ || "$LENGTH" -le 0 ]]; then
  echo "WARNING: invalid LENGTH for K1=$CUTOFF K2=$CUTOFF2, skipping this combination" >&2
  rm -f totaluniqseq uniq.full.fasta "uniq.k.$CUTOFF.c.$CUTOFF2.seqs"
  echo "NA"
  return 0
fi
...
MaxLen=$(mawk '!/^>/' uniq.full.fasta | mawk '(NR==1 || length($0) < shortest){shortest=length($0)} END {if (NR>0) print shortest}')

if [[ -z "${MaxLen:-}" || ! "${MaxLen}" =~ ^[0-9]+$ || "$MaxLen" -le 0 ]]; then
  echo "WARNING: invalid MaxLen for K1=$CUTOFF K2=$CUTOFF2, skipping this combination" >&2
  rm -f totaluniqseq uniq.full.fasta uniq.fq "uniq.k.$CUTOFF.c.$CUTOFF2.seqs"
  echo "NA"
  return 0
fi
...
awk '$4 ~ /^[0-9]+$/ {print $4}' kopt.data > plot.kopt.data
gnuplot << \EOF
